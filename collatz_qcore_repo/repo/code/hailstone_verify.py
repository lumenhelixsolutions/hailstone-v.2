...
def verify_quaternionic(limit: int,
    ...
    # PASS = correct convergence set, regardless of timeouts.
    # Timeouts mean orbits haven't hit the divergence threshold yet;
    # they are expected for non-power-of-2 orbits at small limits.
    hard_fail = bool(extra) or bool(missing)
    status = "✓  PASS" if not hard_fail else "✗  FAIL"
    timeout_note = f"  ({timeouts} orbits still active at step limit — " \
                   f"expected for non-powers at small limits)" if timeouts else ""
    print(f"  {status}")
    ...
    print(f"  Timeouts:  {timeouts}{timeout_note}")
